...
from SegBioModel import SegBioModel
...
class ModelSpec(BaseModel):
    ...
    config: Optional[str] = Field(..., example="2d")
...
def initialize(spec: ModelSpec) -> Dict[str, Any]:
    ...
    return {
        "status": "ok",
        "init_txt": str(scripts_dir / f"SegBioModel_init_Dataset{spec.dataset_id}.txt"),
    }
...
@app.post("/validate")
def validate(spec: ModelSpec) -> Dict[str, Any]:
    """
    ...
    return {
        "dataset_id": spec.dataset_id,
        "mean_dice": mean,
        "validate_txt": str(scripts_dir / f"SegBioModel_validate_Dataset{spec.dataset_id}.txt")
    }
